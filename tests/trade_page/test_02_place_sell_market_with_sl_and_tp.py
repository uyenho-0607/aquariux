...
def test(pages, order_name, test_teardown):
    ...
    sl, tp = calculate_sl_tp(pages.trade_page.get_buy_price(), is_selling=True)

    logger.info(f"Step: Make {order_type!r} order with stop loss: {sl!r} and take profit: {tp!r}")
    pages.trade_page.place_sell_order(order_type, sl, tp)

    logger.info("Verify placed order succeeded notification")
    pages.home_page.notifications.verify_market_order_placed(order_name, 1, sl, tp, is_selling=True)

    logger.info("Verify open position details")
    pages.trade_page.verify_latest_open_position_details(1, tp, sl, is_sell=True)
